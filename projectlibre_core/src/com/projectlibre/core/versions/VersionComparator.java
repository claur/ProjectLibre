...
*/
package com.projectlibre.core.versions;
...
public class VersionComparator implements Comparator<String>{
	...
	public int compare(String o1, String o2) {
		StringTokenizer v1Tok=new StringTokenizer(o1.trim(),"_");
		StringTokenizer v2Tok=new StringTokenizer(o2.trim(),"_");
		String v1="";
		String v2="";
		String v1Suffix="";
		String v2Suffix="";
		if (v1Tok.hasMoreElements()) v1=v1Tok.nextToken().trim();
		if (v1Tok.hasMoreElements()) v1Suffix=v1Tok.nextToken().trim().toLowerCase();
		if (v2Tok.hasMoreElements()) v2=v2Tok.nextToken().trim();
		if (v2Tok.hasMoreElements()) v2Suffix=v2Tok.nextToken().trim().toLowerCase();
		
		StringTokenizer v1Tok2=new StringTokenizer(v1,".");
		StringTokenizer v2Tok2=new StringTokenizer(v2,".");
		while (v1Tok2.hasMoreTokens() || v2Tok2.hasMoreTokens()){
			int i1=v1Tok2.hasMoreTokens()? Integer.parseInt(v1Tok2.nextToken().trim()) :0;
			int i2=v2Tok2.hasMoreTokens()? Integer.parseInt(v2Tok2.nextToken().trim()) :0;
			if (i1<i2) return -1;
			else if (i1>i2) return 1;
		}
		if (v1Suffix.length()==0 && v2Suffix.length()==0)
			return 0;
		if (v1Suffix.length()==0 && v2Suffix.length()>0)
			return 1;
		if (v1Suffix.length()>0 && v2Suffix.length()==0)
			return -1;
		int i=v1Suffix.compareTo(v2Suffix);
		if (i<0) return -1;
		if (i>0) return 1;
		return 0;
	}
